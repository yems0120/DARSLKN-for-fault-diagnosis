...
class Bottleneck(nn.Module):  
    def __init__(self, channel):
        super().__init__()
        med_channel = channel*4
        self.cv1 = Conv(channel, med_channel, 1)
        self.cv2 = Conv(med_channel, med_channel, 3, p=1)
        self.cv3 = Conv(med_channel, channel, 1)
...
class AR_CSP101(nn.Module):
    def __init__(self, in_channels=1, classes=5):
        super(AR_CSP101, self).__init__()
        self.stem = nn.Sequential(
            Conv(in_channels, 12, k=7, s=2, p=3, act=True),
            torch.nn.MaxPool1d(3,2,1),
            )
        self.csp1 = AR_CSPBlock(12, 30, n=3)
        self.csp2 = AR_CSPBlock(30, 42, n=4)
        self.csp3 = AR_CSPBlock(42, 42, n=6, dowsample=False)
        self.csp4 = AR_CSPBlock(42, 42, n=6, dowsample=False)
        self.csp5 = AR_CSPBlock(42, 42, n=6, dowsample=False)
        self.csp5 = AR_CSPBlock(42, 60, n=5)
        self.csp6 = AR_CSPBlock(60, 60, n=3, dowsample=False)        
